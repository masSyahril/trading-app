...
/** Sub-chart colors when <html data-tryout-chart-theme="tradingview"> (newstyle.html). */
function getTryoutEmbeddedChartTheme() {
  try {
    if (typeof document !== 'undefined' && document.documentElement.dataset.tryoutChartTheme === 'tradingview') {
      return {
        layoutBg: '#131722',
        textColor: '#d1d4dc',
        gridVert: '#363c4e',
...
class MultiIndicatorSystem {
  ...
  createIndicatorDefinitions() {
    return {
      ...
      HULLHMA: {
          ...
          render: (chart, data, colors, seriesMap) => this.renderHULLHMA(chart, data, colors, seriesMap)
        },
      VOLMA: {
  ...
  initializePanel(panelId, indicatorType) {
    ...
    const _th = getTryoutEmbeddedChartTheme();
    const chart = LightweightCharts.createChart(chartElement, {
      layout: {
        background: { color: _th.layoutBg },
        textColor: _th.textColor
      },
      grid: {
        vertLines: { color: _th.gridVert },
        horzLines: { color: _th.gridHorz }
      },
      rightPriceScale: {
        borderColor: _th.scaleBorder,
