...
class MultiIndicatorSystem {
  ...
  seriesWithLeadInPadding(dataArray, valueFn = (v) => v) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      let out = valueFn(values[i], i);
      if (typeof out === 'number' && !Number.isFinite(out)) out = null;
      if (out != null && typeof out === 'object' && 'value' in out) {
        let v = out.value;
        if (typeof v === 'number' && !Number.isFinite(v)) v = null;
        result.push({ time: c.time, ...out, value: v });
      } else {
  ...
  createIndicatorDefinitions() {
    return {
      ...
      RWI: {
        ...
        minPeriod: 14,
        isNew: true, // Set to true when you add a new indicator; remove or set false when no longer "new"
      ...
      DEMA: {
        ...
        paramLabels: { period: 'Period' },
        minPeriod: 2,
        compute: (data, params) => this.computeDEMAIndicator(data, params.period),
  ...
  createFlexiblePanel(panelId, containerId, defaultIndicator, panelNumber) {
    ...
    const panelHTML = `
      <div class="indicator-panel flexible" id="${panelId}" data-indicator="${defaultIndicator}">
        <div class="indicator-header">
          ...
          <div class="indicator-select-wrap">
            <select class="indicator-select" id="${panelId}-select">
              ${Object.entries(this.indicatorDefinitions)
                .sort((a, b) => a[1].name.localeCompare(b[1].name))
                .map(([key, def]) => 
                  `<option value="${key}" ${key === defaultIndicator ? 'selected' : ''}>${def.name}${def.isNew ? '  NEW' : ''}</option>`
                ).join('')}
            </select>
            <span class="indicator-new-badge indicator-new-badge-hidden" id="${panelId}-new-badge" aria-hidden="true">NEW</span>
          </div>
    ...
    if (panel) {
      ...
      this.setupNewBadgeClick(panelId);
      this.updateNewBadge(panelId);
    }
  ...
  static get NEW_INDICATORS_SEEN_KEY() { return 'trading_app_new_indicators_seen'; }

  isIndicatorNew(indicatorKey) {
    const def = this.indicatorDefinitions[indicatorKey];
    return def && def.isNew === true;
  }

  getNewIndicatorsSeen() {
    try {
      const raw = localStorage.getItem(MultiIndicatorSystem.NEW_INDICATORS_SEEN_KEY);
  ...
  markNewIndicatorSeen(indicatorKey) {
    if (!indicatorKey) return;
    const seen = this.getNewIndicatorsSeen();
    if (seen.includes(indicatorKey)) return;
    seen.push(indicatorKey);
    try {
      localStorage.setItem(MultiIndicatorSystem.NEW_INDICATORS_SEEN_KEY, JSON.stringify(seen));
    } catch (e) {}
  }

  shouldShowNewBadge(indicatorKey) {
    return this.isIndicatorNew(indicatorKey) && !this.getNewIndicatorsSeen().includes(indicatorKey);
  }

  updateNewBadge(panelId) {
    const badge = document.getElementById(`${panelId}-new-badge`);
    if (!badge) return;
    const panel = this.panels.get(panelId);
    const key = panel ? panel.indicatorType : (document.getElementById(`${panelId}-select`)?.value);
    if (this.shouldShowNewBadge(key)) {
      badge.classList.remove('indicator-new-badge-hidden');
      badge.setAttribute('aria-hidden', 'false');
    } else {
      badge.classList.add('indicator-new-badge-hidden');
      badge.setAttribute('aria-hidden', 'true');
    }
  }

  setupNewBadgeClick(panelId) {
    const badge = document.getElementById(`${panelId}-new-badge`);
    if (!badge) return;
    badge.addEventListener('click', (e) => {
      ...
      const panel = this.panels.get(panelId);
      const key = panel ? panel.indicatorType : (document.getElementById(`${panelId}-select`)?.value);
      if (key && this.isIndicatorNew(key)) {
        this.markNewIndicatorSeen(key);
        this.updateNewBadge(panelId);
      }
    });
  }
        ...
        const handleChange = (e) => {
          ...
          if (newValue && newValue !== currentType) {
            try {
              ...
              if (this.isIndicatorNew(newValue)) this.markNewIndicatorSeen(newValue);
              this.updateNewBadge(panelId);
  ...
  _computeHullMAInline(STK_close, day, ema_n = 9) {
    ...
    if (typeof window !== 'undefined' && typeof window.computeHullMAProfWangSparse === 'function') {
      const sp = window.computeHullMAProfWangSparse(STK_close, day, ema_n);
      const n = STK_close.length;
      const HMA = new Array(n).fill(null);
      const eHMA = new Array(n).fill(null);
      for (let i = 0; i < n; i++) {
        if (i in sp.HMA) HMA[i] = sp.HMA[i];
        if (i in sp.eHMA) eHMA[i] = sp.eHMA[i];
      }
      if (n > 0 && n in sp.HMA && !(n - 1 in sp.HMA)) HMA[n - 1] = sp.HMA[n];
      if (n > 0 && n in sp.eHMA && !(n - 1 in sp.eHMA)) eHMA[n - 1] = sp.eHMA[n];
      return { HMA, eHMA };
    }
    return { HMA: [], eHMA: [] };
  }
  ...
  // DEMA Indicator Computation
  computeDEMAIndicator(data, period = 20) {
    const closes = data.map(d => d.close);
    
    if (!window.WangIndicators || !window.WangIndicators.computeDEMA) {
      console.error('WangIndicators.computeDEMA not available');
      return { dema: [] };
    }
    
    const dema = window.WangIndicators.computeDEMA(closes, period);
    return { dema };
  }
  ...
  computeREXOscillator(data, period = 14) {
