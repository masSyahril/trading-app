...
function computeMA(STK_close, Ma_day) { // Ma_day = 5,10,20 etc. Values is an array of closing prices
  ...
  for (i = Ma_day +1; i <= STK_close.length; i++) { 
     //MA(i)==(MA(i-1)*Ma_day-STK_close[i-Ma_day-1] + STK_close[i-1])/Ma_day;
     MA(i)=(MA(i-1)*Ma_day-STK_close[i-Ma_day] + STK_close[i])/Ma_day;
...
function computeRSI(RSI_day1, RSI_day2) {
  ...
  let Ut = 0;
  let Dt = 0;
  //for(let i=2; i<RSI_day1; i++){  // Fahmi輸入此，應該錯誤!
  ...
  Ut = 0;
  Dt = 0;
  //for(let i=2; i<RSI_day2; i++){     // Fahmi輸入此，應該錯誤!
...
function computeARBR(ARBR_day) {
  ...
  for(let i=ARBR_day+1; i<STK_close.length; i++){
...
function computeCR(CR_day) {
  ...
  for(let  i=CR_day+2; i<STK_close.length; i++) {
...
function computeBBI3(day1, day2, day3){
  ...
  return BBI3(), RR, Acc_RR, BS_times;
...
function computeBBI4(day1, day2, day3, day4){
  ...
  console.log(Acc_RR);  //print ACC_RR累積報酬率。
  return BBI4(), RR, Acc_RR, BS_times;
...
function computeBBI5(day1, day2, day3, day4, day5){
  ...
  console.log(Acc_RR);  //print ACC_RR累積報酬率。
  return BBI5(), RR, Acc_RR, BS_times;
...
function computeBIAS(BIAS1,BIAS2, day1, day2){
  ...
  for(let i=day2; i<STK_close.length; i++){
...
function computeATR(smoothed_n) {
  // TR(i) = 2,3,...,2000.
  let ATR = [];  //需要設定嗎？  const ATR = []
  let TR = [];   //需要設定嗎？  const TR = []
  for(let i = 2; i < STK_close.length; i++) {
...
function computeADI(ema_n) {
  ...
  for (let i=2; i<STK_close.length; i++){
    ...
    eADI(i)=(ema_n-1)/(ema_n+1)*eADI(i-1)+2/(ema_n+1)*ADI(i);
...
//function computeHLO(HLO, eHLO, ema_n)
function computeHLO(ema_n) {
  // ema_n=9, 平滑化因子
  const HLO=[];
  const eHLO=[];
  ...
  for(let i=2; i<STK_close.length; i++) {
    ...
    eHLO(i)=(ema_n-1)/(ema_n+1)*eHLO(i-1)+2/(ema_n+1)*HLO(i);
...
function computeVHF(VHF_day) {
  ...
  let sum=0;
  for (let i=1; i<STK_close.length-VHF_day+1; i++) {
    ...
    VHF(i+VHF_day-1)=Math.abs(max-close-min_close)/sum;  //例第一筆=VHF(20)
    if (i>1) {
      VHFs(i+VHF_day-1)=(smoothed-1)/(smoothed+1)*VHF(i+VHF_day-2)+2/(smoothed+1)*VHF(i+VHF_day-1);
    }  //例參數VHF_day=20，則第一筆=VHFs(21)
    sum=0;  //分母的累加要歸零
  }
  return VHF(), VHFs();
...
function computeVolRatio(day, esp) {   //原名=VR
  //一般取day=26日為計算區間
  //const esp=10;平滑的天數exponential smoothing parameter(esp)
  const VolRatio=[];
  const eVolRatio=[];   //這個要宣告嗎？平滑化後的陣列
  ...
  VolRatio[day+1]=(UpVol+TtlVol/2)/(DnVol+TtlVol/2)*100;  //第一個VR(21)
  eVolRatio[day+1]=(esp-1)/(esp+1)*eVolRatio[day]+2/(esp+1)*VolRatio[day+1];
  //第一個平滑VRs(21)。 VRs(21)=(n-1)/(n+1)*VR(20)+2/(n+1)*VR(21)
  for(let i=2+day; i<STK_close.length; i++) {   //let i=22 to 2000, 設day=20
    ...
    if (STK_closee[i]<STK_close[i-1]) {   //下跌
      DnVol=DnVol+STK_vole[i];             //累加第2個下跌成交量STK_vol(22)
    }  
  ...
  //例參數day=20, VolRatio()=21,22,...,2000, eVolRatio()=21,22,...,2000.
}
//----------------------------------------------------------------------
...
function computeDEMA(day) {
  ...
  let sum=0;
  for(let i=1; i<day; i++) {     //例如：let i=1 to 20
    sum=sum+STK_close(i);
  }
  EMA(day)=sum/day;    //第一個EMA(20)
  DEMA(day)=sum/day;   //第一個DEMA(20)
...
function computeADR(ADR_day) {
  ...
  for(let  i=2; i<ADR_day; i++) {    //例：let i=2,3,...,20
  ...
  //計算其餘的ADR=21,22,...,2000
  for(let  i=ADR_day+1; i<STK_close.length; i++) {    //例：let  i=21,22,...,2000.
    //先扣掉第2天比第1天是否漲跌的天數。
...
function computeVRMA(day1, day2) {      //MA_day1, MA_day2
  const VRMA1=[], VRMA2=[];             //例：VRMA1=5日MA變動率，VRMA2=10的MA變動率
  const MA1 = computeMA(STK_close, day1);
  const MA2 = computeMA(STK_close, day2);
...
function computeIMI(day1, day2) {
  const IMI1=[], IMI2=[];
  let Iup, Idn = 0;
  for (let i=1; i<day1; i++) {     //例: let  i=1 to 10
  ...
  for (let i=day1+1; i<STK_close.length; i++) {  //let i=11,12,...,2000.
    //先扣掉第1天比第1天是否C>O。
...
//function computeMTM(K_close, day1, day2) { //原有2個MTM
//const esp=10;平滑的天數exponential smoothing parameter(esp)
function computeMTM(K_close, day, esp) {
  //K_close=STK_close[], for example: day=10, esp=9,10, ...
...
function computeROC(day1, day2) {
  const ROC1=[], ROC2=[];
  for (let i=day1; STK_close.length; i++) {   //ROC1()
  ...
  for (let i=day2; STK_close.length; i++) {   //ROC2()
...
function computeKST(day1, day2, day3, day4) {
  ...
  for(let i=day1; STK_close.length; i++) {   //ROC1(),10天
    //ROC1=10,11,...,2000.
    ROC1(i)=(STK_close(i)/STK_close(i-day1+1)-1)*100; }
  for(let i=day2; STK_close.length; i++) {   //ROC2(),15天
    //ROC2=15,16,...,2000.
    ROC2(i)=(STK_close(i)/STK_close(i-day2+1)-1)*100; }
  for(let i=day3; STK_close.length; i++) {   //ROC3(),20天
    //ROC3=20,21,...,2000.
    ROC3(i)=(STK_close(i)/STK_close(i-day3+1)-1)*100; }
  for(let i=day4; STK_close.length; i++) {   //ROC4(),30天
    //ROC4=30,31,...,2000.
  ...
  for(let i=max_day; STK_close.length; i++) {  //let i=30 to 2000
  ...
  for(let  i=max_day+1; STK_close.length; i++) {  //let i=31 to 2000
...
function computeOBV() {
  ...
  for(let  i=2; i<STK_close.length; i++) {   //let i=2 to 2000.
...
function computeACC(MTM_n, ACC_n) {
  const MTM=[], ACC=[];
  for(let i=MTM_n; STK_close.length; i++) {    //MTM()
  ...
  for(let i=MTM_n+ACC_n; STK_close.length; i++) {  //let i=15 to 2000.
...
function computeCostMA(day) {
  ...
  for(let i=1; i<day; i++) {   //例:計算前10筆加總，let i=1 to 10
  ...
  for(let i=day+1; i<STK_close.length; i++) {  //例:let i=11 to 2000
    //先扣除10天前的分子加總、分母加總
...
function computeVolROC(K_vol, roc_length, ma_day) {
...
//  const VROC1=[], VROC2=[];
//  for(let i=day1; STK_close.length; i++) {   //VROC1()
...
//    VROC1(i)=(STK_vol(i)/STK_vol(i-day1+1)-1)*100;  //與上式相同
    ...
    //VROC1=10,11,...,2000.
//  }
//  for(let i=day2; STK_close.length; i++) {   //VROC2()
...
//function computeMFI(day) {  //原來的設計
function computeMoneyFlowIndex(day, esp) {
  //過去day日的正負資金流，例：day=10
  //esp=9,指數平滑移動平均參數,Exponential smoothing parameter(esp)
  const MoneyFlowIndex=[], eMoneyFlowIndex=[];
  let PMF=0, NMF=0; //PositiveMoneyFlow正資金流,NegativeMoneyFlow負資金流
  for(let i=2; i<day; i++) {   //例:i=2 to 10
    let yesterday_TpPrice=(STK_high[i-1]+STK_low[i-1]+STK_close[i-1])/3;
    //TypicalPrice 昨天的
    let today_TpPrice=(STK_high[i]+STK_low[i]+STK_close[i])/3;
    //TypicalPrice 今天的
    ...
    MoneyFlowIndex[day]=PMF/(PMF+NMF)*100;   //第1個MFI值，例：MFI(10)
    eMoneyFlowIndex[day]=PMF/(PMF+NMF)*100;  //初值,指數平滑移動平均eMFI(10)=MFI(10)
  }
  for(let i=day+1; i<STK_close.length; i++) {  //例:i=11 to 2000
    ...
    MoneyFlowIndex[i]=PMF/(PMF+NMF)*100;   //第2個MFI值，例：MFI(11)
    eMoneyFlowIndex[i]=(esp-1)/(esp+1)*eMoneyFlowIndex[i-1]+2/(esp+1)*MoneyFlowIndex[i];
    //eMFI今=(n-1)/(n+1)*eMFI昨+2/(n+1)*MFI今。參數=9,<本人自創>
    //第2個eMFI值，例：eMFI(11), eMFI=11,12,...,2000.
  }
  return MoneyFlowIndex(), eMoneyFlowIndex();
  //drawing the MoneyFlowIndex and eMoneyFlowIndex figures in the small windows.
  //例：day=10, MoneyFlowIndex,eMoneyFlowIndex=10,11,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-15=================================
...
function computePVI() {
  const PVI=[], ePVI=[];
  let ePVI_n=9;      //指數平滑移動平均參數=9
  PVI(1)=100;        //初值
  ePVI(1)=PVI(1);    //初值,指數平滑移動平均
  for(let i=2; i<STK_close.length; i++) {   //i=2 to 2000
    ...
    ePVI(i)=(ePVI_n-1)/(ePVI_n+1)*ePVI(i-1)+2/(ePVI_n+1)*PVI(i);
    //ePVI今=(n-1)/(n+1)*ePVI昨+2/(n+1)*PVI今。參數=9,<本人自創>
  }
  return PVI(), ePVI();
  //drawing the PVI and ePVI figures in the small windows.
  //PVI, ePVI=1,2,3,...,2000.
}
//----------------------------------------------------------------------
...
function computeNVI() {
  ...
  for(let i=2; i<STK_close.length; i++) {   //i=2 to 2000
    ...
    eNVI(i)=(eNVI_n-1)/(eNVI_n+1)*eNVI(i-1)+2/(eNVI_n+1)*NVI(i);
    //eNVI今=(n-1)/(n+1)*eNVI昨+2/(n+1)*NVI今。參數=9,<本人自創>
  }
  return NVI(), eNVI();
...
function computeMass() {
  const EMA1=[], EMA2=[], Mass=[];
  const ema_n=9;    //指數平滑移動平均參數=9
  let temp_sum=0;   //暫時變數,前9天sum(最高價－最低價)等
  for(let i=1; i<ema_n; i++) {   //計算前9筆加總，i=1 to 9
    let temp_sum=temp_sum+(STK_high(i)-STK_low(i));
  }
  EMA1(ema_n)=temp_sum/ema_n;   //第1個EMA1值,EMA1(9)
  EMA2(ema_n)=EMA1(ema_n);      //第1個EMA2值,EMA2(9)=EMA1(9)
  temp_sum=EMA1(ema_n)/EMA2(ema_n);  //暫時變數加總,EMA1(9)/EMA2(9)
  let mass_n=5;  //此處Mass參數N=5 (一般參數設為25天)
  //計算i=10 to 13
  for(let i=ema_n+1; i<ema_n+mass_n-1; i++) {  //i=10 to 13
    EMA1(i)=(ema_n-1)/(ema_n+1)*EMA1(i-1)+2/(ema_n+1)*(STK_high(i)-STK_low(i));
    EMA2(i)=(ema_n-1)/(ema_n+1)*EMA2(i-1)+2/(ema_n+1)*EMA1(i);
    temp_sum=temp_sum+EMA1(i)/EMA2(i);
  }
  Mass(ema_n+mass_n-1)=temp_sum;  //第1個Mass值,Mass(13)=Mass(9+5-1)
  //計算i=14 to 2000
  for(let i=ema_n+mass_n; i<STK_close.length; i++) {  //i=14 to 2000
    EMA1(i)=(ema_n-1)/(ema_n+1)*EMA1(i-1)+2/(ema_n+1)*(STK_high(i)-STK_low(i));
    EMA2(i)=(ema_n-1)/(ema_n+1)*EMA2(i-1)+2/(ema_n+1)*EMA1(i);
    //要移除第(i-5=9)天的EMA1/EMA2,即5天前的EMA1/EMA2
    temp_sum=temp_sum+EMA1(i)/EMA2(i)-EMA1(i-mass_n)/EMA2(i-mass_n);
    Mass(i)=temp_sum;  //Mass=14,15,...,2000.
  }
  return EMA1(), EMA2(), Mass();
  //drawing the EMA1, EMA2 and Mass figures in the small windows.
  //EMA1,EMA2=9,10,...,2000.
  //Mass=13,14,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-16=================================
//OSCP(Price Oscillator,價格擺動指標)
//OSCP今=(短期MA今－長期MA今)/短期MA今*100%
//eOSCP今=(n-1)/(n+1)*eOSCP昨+2/(n+1)*OSCP今。參數=9,<本人自創>
function computeOSCP(short_day, long_day) {
  const OSCP=[], eOSCP=[];
  //const shortMA=[], longMA=[]; //不需要宣告嗎？
  let eOSCP_n=9;      //指數平滑移動平均參數=9
  if(short_day>long_day) {  //例如: 10>5, 將二者對調,確保short_day比較小。
    let temp=short_day;
    short_day=long_day;
    long_day=temp;
  }
  const shortMA = computeMA(STK_close, short_day);  //例如5天MA
  const longMA = computeMA(STK_close, long_day);    //例如10天MA
  for(let i=long_day; i<STK_close.length; i++) {    //i=10 to 2000
    OSCP(i)=(shortMA(i)-longMA(i))/shortMA(i)*100;      //計算第1個OSCP(10)
    if(i==long_day) {      //令eOSCP初值=OSCP初值, 例i=10
      eOSCP(i)=OSCP(i); }
    else {                // i>10
      eOSCP(i)=(eOSCP_n-1)/(eOSCP_n+1)*eOSCP(i-1)+2/(eOSCP_n+1)*OSCP(i);
    }
  }
  return OSCP(), eOSCP();
  //drawing the OSCP and eOSCP figures in the small windows.
  //例如long_day=10天，OSCP, eOSCP=10,11,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-16=================================
//OSCV(Volume Oscillator,成交量擺動指標)
//OSCV今=(短期MA今－長期MA今)/短期MA今*100%
//eOSCV今=(n-1)/(n+1)*eOSCV昨+2/(n+1)*OSCV今。參數=9,<本人自創>
function computeOSCV(short_day, long_day) {
  const OSCV=[], eOSCV=[];
  //const shortMA=[], longMA=[]; //不需要宣告嗎？
  let eOSCV_n=9;      //指數平滑移動平均參數=9
  if(short_day>long_day) {  //例如: 10>5, 將二者對調,確保short_day比較小。
    let temp=short_day;
    short_day=long_day;
    long_day=temp;
  }
  const shortMA = computeMA_vol(STK_vol, short_day);  //例如5天MA
  const longMA = computeMA_vol(STK_vol, long_day);    //例如10天MA
  for(let i=long_day; i<STK_close.length; i++) {    //i=10 to 2000
    OSCV(i)=(shortMA(i)-longMA(i))/shortMA(i)*100;   //計算第1個OSCV(10)
    if(i==long_day) {      //令eOSCV初值=OSCV初值, 例i=10
      eOSCV(i)=OSCV(i); }
    else {                // i>10
      eOSCV(i)=(eOSCV_n-1)/(eOSCV_n+1)*eOSCV(i-1)+2/(eOSCV_n+1)*OSCV(i);
    }
  }
  return OSCV(), eOSCV();
  //drawing the OSCV and eOSCV figures in the small windows.
  //例如long_day=10天，OSCV, eOSCV=10,11,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-16=================================
//MA_vol(Moving Average Volume,移動平均成交量)
function computeMA_vol(STK_vol, day) {
  const MA_vol = [];
  let sum = 0;
  for (let i = 1; i <= day; i++) {
    sum=sum+STK_vol(i);
...
//===designed by Prof Wang, 2025-Dec-17=================================
...
function computeADLine() {
  const ADLine=[];
  ADLine[1]=0;   //初值,第1個ADLine值(1)=0。避免除以0的錯誤 
  //計算第2個ADLine值(2)
  if(STK_high[2]-STK_low[2]==0) {  //分母為0,避免除以0的錯誤
    ADLine[2]=(STK_close[2]/STK_close[1]-1)*STK_vol[2]+ADLine[1]; } //((Ct/Ct-1)-1)*Volt
  else {
    ADLine[2]=(2*STK_close[2]-STK_high[2]-STK_low[2])/(STK_high[2]-STK_low[2])*STK_vol[2]+ADLine[2];
  }
  //計算i=3 to 2000
  for(let i=3; i<STK_close.length; i++) {   //i=3 to 2000
    if(STK_high[i]-STK_low[i]==0) {  //分母為0,避免除以0的錯誤
      ADLine[i]=(STK_close[i]/STK_close[i-1]-1)*STK_vol[i]+ADLine(i-1); }
    else {
      ADLine[i]=(2*STK_close[i]-STK_high[i]-STK_low[i])/(STK_high[i]-STK_low[i])*STK_vol[i]+ADLine[i-1];
    } 
  }
  return ADLine;
  //drawing the ADLine figure in the small windows.
  //ADLine=1,2,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-18=================================
//Chaikin Oscillator蔡金擺動指標
//ChaikinOscillator=短期EMA(ADLine)-長期EMA(ADLine)
// =EMA(ADLine, short_day)-EMA(ADLine, long_pday)
function computeChaikinOscillator(short_day, long_day) {  
  ...
  for(let i=long_day; i<STK_close.length; i++) {   //i=10 to 2000
    ...
    if(i==long_day) {      //令eChaikinOscillator初值=ChaikinOscillator初值, 例i=10
      ...
      eChaikinOscillator(i)=(eCO_n-1)/(eCO_n+1)*eChaikinOscillator(i-1)+2/(eCO_n+1)*ChaikinOscillator(i);
    }
  }
...
function computeChaikinMoneyFlow(CMF_day) {
  //e.g., CMF_day=20
  const CMF=[], eCMF=[];      //例如：=20,21,...,2000
  let eCMF_n=9;      //指數平滑移動平均參數=9, eCMF_n=9。 <本人自創>
  let sum_CLV, sum_Vol=0;    //分子與分母暫時變數,初值=0
  //先加總前20天的sum(今CLV X 今Vol)/sum(今Vol), 分子與分母
  for(let i=1; i<CMF_day; i++) {   //例:i=1 to 20
    sum_CLV=sum_CLV+((2*STK_close(i)-STK_high(i)-STK_low(i))/(STK_high(i)-STK_low(i)))*STK_vol(i);
    sum_Vol=sum_Vol+STK_vol(i);
  }
  //計算第1個CMF值, 例:CMF(20)
  CMF(CMF_day)=sum_CLV/sum_Vol;   
  eCMF(CMF_day)=CMF(CMF_day);    //令eCMF初值=CMF初值, 例:i=20
  for(let i=CMF_day+1; i<STK_close.length; i++) {   //例:i=21 to 2000
    //要移除第(i-20)天的CLV X Vol, 即20天前的CLV X Vol
    sum_CLV=sum_CLV-((2*STK_close(i-CMF_day)-STK_high(i-CMF_day)-STK_low(i-CMF_day))/(STK_high(i-CMF_day)-STK_low(i-CMF_day)))*STK_vol(i-CMF_day);
    sum_Vol=sum_Vol-STK_vol(i-CMF_day);
    //加上今天的CLV X Vol, sum_CLV與sum_Vol
    sum_CLV=sum_CLV+((2*STK_close(i)-STK_high(i)-STK_low(i))/(STK_high(i)-STK_low(i)))*STK_vol(i);
    sum_Vol=sum_Vol+STK_vol(i);
    CMF(i)=sum_CLV/sum_Vol;   //計算CMF值, 例:CMF(21), CMF=21,22,...,2000.
    eCMF(i)=(eCMF_n-1)/(eCMF_n+1)*eCMF(i-1)+2/(eCMF_n+1)*CMF(i);
    //eCMF今=(n-1)/(n+1)*eCMF昨+2/(n+1)*CMF今。參數=9,<本人自創>指數平滑移動平均
  }
  return CMF(), eCMF();
  //drawing the CMF, eCMF figures in the small windows.
  //if CMF_day=20, then CMF, eCMF=20,21,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-20=================================
//ASI累積擺動指標(ASI, Accumulation Swing Index),此指標計算繁瑣
function computeASI(ASIma_n, ASIema_n) {
  //ASI的移動平均=ASIma,天數=ASIma_n,例如=10。
  //ASI的移動平均=ASIma, 指數平滑移動平均=ASI_ema,參數=ASIema_n=9.
  const ASI=[];
  const ASIma=[], ASIema=[];
  let sum_ASI=0;  //加總移動平均ASI的值
  ASI(1)=0;       //第1天的ASI()無資料
  //計算第1天到第ASIma_n天(例如=10天)的ASI,它是從第1天開始累加的
  for(let i=2; i<ASIma_n+1; i++) {   //例如: i=2 to 11,共10天
    let aa=Math.abs(STK_high(i)-STK_close(i-1));
    let bb=Math.abs(STK_low(i)-STK_close(i-1));
    let cc=Math.abs(STK_high(i)-STK_low(i-1));
    let dd=Math.abs(STK_close(i)-STK_open(i-1));
    let ee=STK_close(i)-STK_close(i-1);
    let ff=STK_close(i)-STK_open(i);
    let gg=STK_close(i-1)-STK_open(i-1);
    let mm=ee+ff/2+gg/4;
    let kk=Math.max(aa,bb);
    switch(true) {
    ...
    ASI(i)=ASI(i-1)+kk*mm/rr*50;   //ASI=2,3,...,11.  例:ASIma_n=10天
  }
  ASIma(i)=ASI(i)/ASIma_n; //ASIma=11,第1個ASI的移動平均=ASIma(11)
  ASIema(i)=ASIma(i);      //ASIema=11,第1個ASIma指數平滑移動平均=ASIema(11),令相等。
  sum_ASI=ASI(i);  //加總移動平均ASI的值

  //計算其餘的ASI等例如:i=12 to 2000, ASI的移動平均=ASIma,天數=ASIma_n=10天
  for(let i=ASIma_n+2; i<STK_close.length; i++) {   //例如: i=12 to 2000.
    let aa=Math.abs(STK_high(i)-STK_close(i-1));
    let bb=Math.abs(STK_low(i)-STK_close(i-1));
    let cc=Math.abs(STK_high(i)-STK_low(i-1));
    let dd=Math.abs(STK_close(i)-STK_open(i-1));
    ...
    ASI(i)=ASI(i-1)+kk*mm/rr*50;   //ASI=12,13,...,2000. 例:ASIma_n=10天
    //加總移動平均ASI的值sum_ASI,要先扣除10天前的(ASI(2)),再加上新的ASI(12)
    sum_ASI=sum_ASI-ASI(i-ASIma_n)+ASI(i);
    ASIma(i)=sum_ASI/ASIma_n;   //移動平均ASIma的時間長度=10天
    //ASIma的指數平滑移動平均=ASIema,參數=ASIema_n=9
    ASIema(i)=((ASIema_n-1)/(ASIema_n+1))*ASIema(i-1)+(2/(ASIema_n+1))*ASIma(i);
    //ASIema今=(n-1)/(n+1)*ASIema昨+2/(n+1)*ASIma今。參數=9,<本人自創>指數平滑移動平均
  }
  return ASI(), ASIma(i), ASIema();
  //drawing the ASI, ASIma, ASIema figures in the small windows.
  //ASI()=2,3,...,2000.    ASIma,ASIema=11,12,...,2000
  //ASI的移動平均=ASIma,天數=ASIma_n=10天,ASIma=11,12,...,2000.
  //設指數平滑移動平均參數=ASIema_n=9,ASIema=11,12,...,2000.
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2025-Dec-20=================================
//TRIX(Triple Exponential)三重指數平滑移動平均指標
//對數據進行三次平滑處理，再根據這條移動平均線的變動情況來預測股價的長期走勢。
function computeTRIX(ema_n) {
  //ema_n=9;           //指數平滑移動平均參數=9
  const EMA1=[], EMA2=[], EMA3=[], EMA4=[], TRIX=[];
  let temp_sum=0;     //暫時變數,前ema_n天sum(close)等
  for(let i=1; i<ema_n; i++) {   //計算前ema_n筆加總，i=1 to 9
    let temp_sum=temp_sum+STK_close(i);
  }
  EMA1(ema_n)=temp_sum/ema_n;   //第1個EMA1值,EMA1(9)
  EMA2(ema_n)=EMA1(ema_n);      //第1個EMA2值,EMA2(9)=EMA1(9)
  EMA3(ema_n)=EMA2(ema_n);      //第1個EMA3值,EMA3(9)=EMA2(9)
  EMA4(ema_n)=EMA3(ema_n);      //第1個EMA4值,EMA4(9)=EMA3(9)
  //計算i=ema_n+1 to 2000
  for(let i=ema_n+1; i<STK_close.length; i++) {  //i=10 to 2000
    EMA1(i)=(ema_n-1)/(ema_n+1)*EMA1(i-1)+2/(ema_n+1)*STK_close(i);
    EMA2(i)=(ema_n-1)/(ema_n+1)*EMA2(i-1)+2/(ema_n+1)*EMA1(i);
    EMA3(i)=(ema_n-1)/(ema_n+1)*EMA3(i-1)+2/(ema_n+1)*EMA2(i);
    //再對EMA3(三次指數平滑移動平均)進行指數平滑移動平均
    EMA4(i)=(ema_n-1)/(ema_n+1)*EMA4(i-1)+2/(ema_n+1)*EMA3(i);
    //計算TRIX值(進行變化率計算)
...
function computeMAoneMAtwo(day1, day2) {    // day1<day2
  let temp1=Math.min(day1, day2);  //確保day1較小
  let temp2=Math.max(day1, day2);  //確保day2較大
  day1=temp1;   //確保day1比較小
  day2=temp2;   //確保day2比較大
  const MA1 = computeMA(STK_close, day1); //例如5天MA1
  const MA2 = computeMA(STK_close, day2); //例如10天MA2
  let RR=0;        //報酬率初值=0
  ...
  for(let i=day1+1; i<STK_close.length; i++) {  //例:i=5+1 to 2000
    if((STK_close(i-1)<MA1(i-1)) & (STK_close(i)>MA1(i)) & bs_flag=="N") {
      let buy_price=STK_close(i); 
      bs_flag="Y"; }
    else if((STK_close(i-1)>MA2(i-1)) & (STK_close(i)<MA2(i))) {
      let sell_price=STK_close(i);
      //compute RR, Acc_RR
  ...
  return Acc_RR, BS_times, Acc_RR/BS_times;
  //累積報酬率，累積買賣次數，平均一次報酬率
}
//----------------------------------------------------------------------

//===designed by Prof Wang, 2026-Jan-09=================================
//Follow-The-Wave Strategy (FWS策略指標)。此處只有MA1均線
//當股價由下往上突破MA1時買進，當股價由上往下突破MA1時賣出
function computeMAone(day1) {
  const MA1 = computeMA(STK_close, day1); //例如5天MA1
  let RR=0;        //報酬率初值=0
  let Acc_RR=0;    //累積報酬率初值=0
  let BS_times=0;  //累積買賣次數=0
...
//===designed by Prof Wang, 2026-Jan-11=================================
//K2D2指標(改良型KD指標)
//K2今=(n-1)/(n+1)*K2昨+2/(n+1)*KD_K今。 指數平滑移動平均
//D2今=(n-1)/(n+1)*D2昨+2/(n+1)*KD_D今
function computeK2D2(KD_day, KD2_day) {
  //KD_day=9, KD2_day=9. KD參數=9，指數平滑移動平均參數可與KD相同或不同
  const KD_K=[];  //傳統的KD
  const KD_D=[];  //傳統的KD
  const K2=[];    //新的K2[]=Kema[]
  const D2=[];   //新的D2[]=Dema[]
  // let RSV=0;     //隨機值RSV初值=0
  for(i=1; i<STK_close.length-KD_day+1; i++) {  //例：i=1 to 2000-KD_day+1
    let KD_max=0;
    let KD_min=999;
    for(let j=i; j<KD_day+i-1; j++) {
      if(STK_high(j)>KD_max) KD_max=STK_high(j);
      if(STK_low(j)<KD_min)  KD_min=STK_low(j);
    }
    if(KD_max==KD_min) {    //如果二者相等，分母為零。
      let RSV=100;   }
    else {
      let RSV=(STK_close(KD_day+i-1)-KD_min)/(KD_max-KD_min)*100;
    }
    if(i==KD_day) {
      KD_K(KD_day-1)=50;    //initial value, KD_K(8)=50
      KD_D(KD_day-1)=50;    //initial value, KD_D(8)=50
    }
    KD_K(KD_day+i-1)=2/3*KD_K(KD_day+i-2)+1/3*RSV;
    KD_D(KD_day+i-1)=2/3*KD_D(KD_day+i-2)+1/3*KD_K(KD_day+i-1);
    //K2(9)與D2(9)初值，令等於KD_K(9)，KD_D(9)
    //K2今=(n-1)/(n+1)*K2昨+2/(n+1)*KD_K今。 指數平滑移動平均
    //D2今=(n-1)/(n+1)*D2昨+2/(n+1)*KD_D今
    if(i=KD_day) {
      K2(KD_day)=KD_K(KD_day);
      D2(KD_day)=KD_D(KD_day);  }
    else {
      K2(i)=(KD2_day-1)/(KD2_day+1)*K2(i-1)+2/(KD2_day+1)*KD_K(i);
      D2(i)=(KD2_day-1)/(KD2_day+1)*D2(i-1)+2/(KD2_day+1)*KD_D(i);
      // if KD_day=9, K2[], D2[]=9 to 2000
    }
  }
  //Return KD_K, KD_D, K2, D2;
  return {K2, D2};
  // drawing the K2 and D2 figures in the small windows.
  // if KD_day=9, K2[], D2[]=9 to 2000
}
//----------------------------------------------------------------------
...
function computeCoppockCurve(short_day, long_day, wgt_day) {
  const short_ROC = [];
  const long_ROC = [];
  const composite_ROC = [];
  const Coppock = [];
  const eCoppock = [];  //eCoppock是Coppock的平滑化，即:(n-1)/(n+1)*昨+2/(n+1)*今
...
//===designed by Prof Wang, 2026-Jan-15=================================
...
function computeVolMA(day) {
  const VolMA=[];  //移動平均成交量
  let sum=0;
  for(i=1; i<day; i++) {
    sum=sum+STK_vol[i];
  }
  VolMA(day)=sum/day;  //第1個移動平均成交量=VolMA(10),例: day=10
  for(i=day+1; i<STK_close.length; i++) { //i=11 to 2000.
    sum=sum-STK_vol[i-day]+STK_vol[i];    //先減10天前的+今天第11天的
    VolMA(i)=sum/day;                     //第2個VolMA(11) 
  }
  return VolMA();
  //drawing the VolMA[] figure in the STK_Vol window.
  //day=10, VolMA[]=10,11,...,2000
}
//----------------------------------------------------------------------
...
function computeM3() {
  const M3=[], eM3=[];
  let num=9;     //指數平滑的天數
  const MA5 = computeMA(STK_close,5);
  const MA10 = computeMA(STK_close,10);
  const MA15 = computeMA(STK_close,15);
  const MA20 = computeMA(STK_close,20);
  for(i=20; i<STK_close.length; i++) {
    M3(i)=STK_close[i]-(MA5[i]+MA10[i]+MA15[i]+MA20[i])/4;
    if(i===20) {
      eM3(i)=M3(i); }    //eM3初值=M3初值
    else {
      eM3(i)=(num-1)/(num+1)*eM3(i-1)+2/(num+1)*M3(i);  //指數平滑num=9
    }  
  }
  return M3(), eM3();
...
//本程式AMA採用EMA處理DMA, 
function computeDMA(short_day, long_day, ema_n) {
  //指數移動平均,係數=ema_n=9
  ...
  const MAshort=computeMA(STK_close, short_day);
  const MAlong=computeMA(STK_close, long_day);
  for(i=long_day; i<STK_close.length; i++) {   //例如: i=20 to 2000
    DMA(i)=MAshort[i]-MAlong[i];   //DMA=短MA-長MA
    if(i===long_day) {
      AMA(i)=DMA(i); }    //初值令相等
    else {                //指數移動平均,係數=ema_n=9
      AMA(i)=(ema_n-1)/(ema_n+1)*AMA(i-1)+2/(ema_n+1)*DMA(i); }
  }
  return DMA(), AMA();
  //drawing the DMA[] and AMA[] figures in the small windows.
  //for example: DMA[] , AMA[]=20,21,...,2000
}
//----------------------------------------------------------------------
...
function computeKingEMA(values, esp) {
  ...
  for(let i=esp+1; i<values.length; i++) {  //i=11 to 2000
    EMA[i]=(esp-1)/(esp+1)*EMA[i-1]+2/(esp+1)*values[i];
  ...
  return EMA, simpleEMA;
  // if esp=ema_n=10, then EMA[]=10,11,...,2000
  // simpleEMA[]=1,2,...,2000
