...
// Simple moving average per bar (null until window filled). Used by Wang MAoneMAtwo via window.KingMA.
function KingMA(values, period) {
  if (!Array.isArray(values) || values.length === 0 || period <= 0) return [];
  const sm = computeSMA(values, period);
  const out = new Array(values.length).fill(null);
  for (let i = period - 1; i < values.length; i++) {
    const v = sm[i - (period - 1)];
...
//=== Hull Moving Average (HMA) — implementation lives in Wang_design__HullMA_*.js (window.computeHullMA). ===
